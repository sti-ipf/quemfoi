...
course = Course.create(:identifier => \"#{c.identifier}\", :description => \"#{c.description}\")"
  puts "course = Course.create(:identifier => \"#{c.identifier}\", :description => \"#{c.description}\")"
  participants = []
...
puts "participant = Participant.create(:name => \"#{p.name}\", :group => \"#{p.group}\", :unit => \"#{p.unit}\",
  :contact => \"#{p.contact.gsub('/','-')}\", :course => course)"
      ...
      puts "ActivitiesParticipant.create(:activity => activity, :participant => participant)"
    end
  end
end
file.close
